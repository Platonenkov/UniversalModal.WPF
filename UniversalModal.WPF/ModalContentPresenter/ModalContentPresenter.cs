...
{
    /*Непосредственно модальное окно, взято где-то в интернете и немного доработано*/
    ...
    public class ModalContentPresenter : FrameworkElement
    {
        ...
        private Panel _LayoutRoot;                               // Used to layout the content and modal content.
        private ContentPresenter _PrimaryContentPresenter;       // Hosts the primary content.
        private ContentPresenter _ModalContentPresenter;         // Hosts the modal content.
         ...
         */
        private static readonly TraversalRequest __TraversalDirection;
        ...
        public static readonly DependencyProperty OverlayBrushProperty =
            DependencyProperty.Register("OverlayBrush", typeof(Brush), typeof(ModalContentPresenter),
                new UIPropertyMetadata(new SolidColorBrush(Color.FromArgb(100, 169, 169, 169)),
        ...
        {
            var show_modal_command_binding = new CommandBinding(ModalContentCommands.ShowModalContent, (sender, args) =>
                this.ShowModalContent(), (sender, args) => args.CanExecute = !this.IsModal);
            this.CommandBindings.Add(show_modal_command_binding);

            var hide_modal_command_binding = new CommandBinding(ModalContentCommands.HideModalContent, (sender, args) =>
                this.HideModalContent(), (sender, args) => args.CanExecute = this.IsModal);
            this.CommandBindings.Add(hide_modal_command_binding);
        }
        ...
        static ModalContentPresenter() => __TraversalDirection = new TraversalRequest(FocusNavigationDirection.First);
        ...
        public ModalContentPresenter()
        {
            ...
            _Overlay.Child = _ModalContentPresenter;
        ...
        protected virtual void OnPreviewModalContentShown(RoutedEventArgs e) => RaiseEvent(e);

        /// <summary>
        /// Raises the ModalContentPresenter.ModalContentShown routed event.
        /// </summary>
        /// <param name="e">The event data.</param>
        protected virtual void OnModalContentShown(RoutedEventArgs e) => RaiseEvent(e);

        /// <summary>
        /// Raises the ModalContentPresenter.PreviewModalContentHidden routed event.
        /// </summary>
        /// <param name="e">The event data.</param>
        protected virtual void OnPreviewModalContentHidden(RoutedEventArgs e) => RaiseEvent(e);

        /// <summary>
        /// Raises the ModalContentPresenter.ModalContentHidden routed event.
        /// </summary>
        /// <param name="e">The event data.</param>
        protected virtual void OnModalContentHidden(RoutedEventArgs e) => RaiseEvent(e);
        ...
        private static void OnIsModalChanged(DependencyObject d, DependencyPropertyChangedEventArgs e)
        {
            ...
            if ((bool)e.NewValue)
            {
                 ...
                 */
                control._CachedTabNavigationMode = KeyboardNavigation.GetTabNavigation(control._PrimaryContentPresenter);
                control._CachedDirectionalNavigationMode = KeyboardNavigation.GetDirectionalNavigation(control._PrimaryContentPresenter);
                control._CachedFocusedElement = Keyboard.FocusedElement;

                KeyboardNavigation.SetTabNavigation(control._PrimaryContentPresenter, KeyboardNavigationMode.None);
                KeyboardNavigation.SetDirectionalNavigation(control._PrimaryContentPresenter, KeyboardNavigationMode.None);
                ...
                control._Overlay.MoveFocus(__TraversalDirection);
            ...
            else
            {
                 ...
                 */
                KeyboardNavigation.SetTabNavigation(control._PrimaryContentPresenter, control._CachedTabNavigationMode);
                KeyboardNavigation.SetDirectionalNavigation(control._PrimaryContentPresenter, control._CachedDirectionalNavigationMode);

                Keyboard.Focus(control._CachedFocusedElement);

                control._PrimaryContentPresenter.MoveFocus(__TraversalDirection);
        ...
        private static void OnModalContentChanged(DependencyObject d, DependencyPropertyChangedEventArgs e)
        {
             ...
             * when it is requested by WPF.
             */
            control._ModalContentPresenter.Content = e.NewValue;
            control.AddLogicalChild(e.NewValue);
        ...
        protected override Visual GetVisualChild(int index) => index < 0 || index > 1 ? throw new ArgumentOutOfRangeException("index") : _LayoutRoot;
        ...
        class ModalContentPresenterPanel : Panel
        {
            protected override Size MeasureOverride(Size AvailableSize)
            {
                var result_size = new Size(0, 0);

                foreach (UIElement child in Children)
                {
                    child.Measure(AvailableSize);
                    result_size.Width = Math.Max(result_size.Width, child.DesiredSize.Width);
                    result_size.Height = Math.Max(result_size.Height, child.DesiredSize.Height);
                }
